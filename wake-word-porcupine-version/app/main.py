...
TRANSCRIPTION_SERVICE_URL = os.getenv('TRANSCRIPTION_SERVICE_URL', 'http://192.168.1.88:5000/transcribe')  # Servicio de transcripción
...
class VoiceAssistant:
    ...
    def _send_to_transcription_service(self, wav_bytes: bytes) -> Optional[str]:
        """Enviar audio al servicio de transcripción HTTP local"""
        if not wav_bytes:
            return None
    ...
    def _handle_voice_command(self):
        ...
        if raw_audio:
            ...
            # Transcribir con servicio HTTP local
